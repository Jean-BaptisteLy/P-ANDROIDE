...
static const uint8_t nbre_initial_agents_tetus = 6; //12 = 3 têtus ; 16 = 4 têtus
...
void affichage_resultats() {
  ...
  if (consensus_courant == consensus_atteint) {
    ...
    printf("tableau_nbre_agents[0] = %d \n", tableau_nbre_agents[0]);
    printf("tableau_nbre_agents[1] = %d \n", tableau_nbre_agents[1]);
    printf("tableau_nbre_agents[2] = %d \n", tableau_nbre_agents[2]);
    printf("a = %f \n",fabs(nbre_theorique_a - tableau_nbre_agents[0]));
    printf("a = %f \n",(nbre_theorique_a - fabs(nbre_theorique_a - tableau_nbre_agents[0])));
...
void loop() {
  ...
  if(kilo_uid == 0 && (kilo_ticks % intervalle_temps_kiloticks == 0 || kilo_ticks % intervalle_temps_kiloticks == 1) && kilo_ticks != 1) {
    ...
    double nbre_theorique_b = (double)consensus_atteint * ((double)quality_of_site_b / quality_total);
    double nbre_theorique_c = (double)consensus_atteint * ((double)quality_of_site_c / quality_total);
    qualite_consensus_a[mydata->indice_temps] = fabs((nbre_theorique_a - fabs(nbre_theorique_a - tableau_nbre_agents[0])) / nbre_theorique_a);
    qualite_consensus_b[mydata->indice_temps] = fabs((nbre_theorique_b - fabs(nbre_theorique_b - tableau_nbre_agents[1])) / nbre_theorique_b);
    qualite_consensus_c[mydata->indice_temps] = fabs((nbre_theorique_c - fabs(nbre_theorique_c - tableau_nbre_agents[2])) / nbre_theorique_c);
