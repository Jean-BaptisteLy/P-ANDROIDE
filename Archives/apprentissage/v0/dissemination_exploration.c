...
static const uint32_t taille_ressource = 230;
...
// variables globales
uint8_t consensus_site_a = 0;
uint8_t consensus_site_b = 0;
uint8_t consensus_courant = consensus_site_a + consensus_site_b;
...
void loop() {
  ...
  if(kilo_uid == 0 && (kilo_ticks % intervalle_temps_kiloticks == 0 || kilo_ticks % intervalle_temps_kiloticks == 1) && kilo_ticks != 1) {
    ...
    double nbre_theorique_c = (double)consensus_atteint * ((double)quality_of_site_c / quality_total);
    double nbre_theorique_d = (double)consensus_atteint * ((double)quality_of_site_d / quality_total);
    qualite_consensus_a[mydata->indice_temps] = fabs((nbre_theorique_a - abs(nbre_theorique_a - tableau_nbre_agents[0])) / nbre_theorique_a);
    qualite_consensus_b[mydata->indice_temps] = fabs((nbre_theorique_b - abs(nbre_theorique_b - tableau_nbre_agents[1])) / nbre_theorique_b);
    qualite_consensus_c[mydata->indice_temps] = fabs((nbre_theorique_c - abs(nbre_theorique_c - tableau_nbre_agents[2])) / nbre_theorique_c);
    qualite_consensus_d[mydata->indice_temps] = fabs((nbre_theorique_d - abs(nbre_theorique_d - tableau_nbre_agents[3])) / nbre_theorique_d);
