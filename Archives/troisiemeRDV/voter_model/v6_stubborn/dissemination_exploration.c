...
static const uint8_t nbre_initial_agents_tetus = 2;
static const uint8_t seuil_changements_opinions = 0;
...
void set_opinion_a() {
  if (!mydata->tetu) {
    ...
    //mydata->behavior_state = PHOTOTAXIS;
    mydata->quality_of_site = quality_of_site_a;
    if (kilo_ticks >= 250000) mydata->quality_of_site = quality_of_site_a_bis;
...
void set_opinion_b() {
  if (!mydata->tetu) {
    ...
    //mydata->behavior_state = ANTI_PHOTOTAXIS;
    mydata->quality_of_site = quality_of_site_b;
    if (kilo_ticks >= 250000) mydata->quality_of_site = quality_of_site_b_bis;
