...
void loop() {
  if(mydata->state == DISSEMINATION) {
    if (mydata->new_message) {
      ...
      collisions();
      nest();
      if(mydata->flag_nest && !mydata->flag_voisin_deja_rencontre) {     
