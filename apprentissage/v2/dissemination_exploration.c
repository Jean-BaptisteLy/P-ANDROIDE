...
void mEDEA() {
  ...
  if (kilo_ticks > (mydata->last_time_update + nb_ticks - 93)) { // during the last three seconds
    if (kilo_ticks > (mydata->last_time_update + nb_ticks)) {
      ...
      printf("mydata->genomeList[selection_operator][0] : %d \n", mydata->genomeList[selection_operator][0]);
      printf("mydata->genomeList[selection_operator][1] : %d \n", mydata->genomeList[selection_operator][1]);

      printf("proba_a : %d \n", proba_a);
      printf("proba_b : %d \n", proba_b);

      if (opinion_a_courante > genome_theorique[0] && opinion_b_courante < genome_theorique[1]) {
        proba_a -= penalite_recompense;
        proba_b += penalite_recompense;
      }
      ...
      tirage = selection_operator = (int)(rand() / (double)RAND_MAX * (nbre_kilobots - 1));
      printf("tirage : %d \n",tirage);
      printf("proba_a : %d \n", proba_a);
      printf("proba_b : %d \n", proba_b);
