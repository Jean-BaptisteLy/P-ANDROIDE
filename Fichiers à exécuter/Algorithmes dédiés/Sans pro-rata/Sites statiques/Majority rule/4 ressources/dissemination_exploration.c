...
static const uint8_t seuil_majority_rule = 60;
...
double nbre_theorique_b = 0;
double nbre_theorique_c = 0;
double nbre_theorique_d = 100;
...
void explore() { 
  if (kilo_ticks > (mydata->last_time_update + mydata->exploration_time)) {
    ...
    else if(mydata->flag_speaker == 3) set_color(RGB(0,0,3));
    else if(mydata->flag_speaker == 4) set_color(RGB(3,3,3));
...
void affichage_resultats() {
  if (consensus_atteint == tableau_nbre_agents[0] || consensus_atteint == tableau_nbre_agents[1] || consensus_atteint == tableau_nbre_agents[2] || consensus_atteint == tableau_nbre_agents[3]) {
    ...
    if ((double)tableau_nbre_agents[3] / 2.0 <= nbre_theorique_d) 
      qualite_consensus_d[dernier_indice_temps] = fabs((nbre_theorique_d - fabs(nbre_theorique_d - tableau_nbre_agents[3])) / nbre_theorique_d);
    else qualite_consensus_d[dernier_indice_temps] = 0.0;
...
void majority_tule() {
  ...
  if (kilo_ticks > (mydata->last_time_update + nb_ticks - 93)) { // during the last three seconds
    if (kilo_ticks > (mydata->last_time_update + nb_ticks)) {
      mydata->last_time_update = kilo_ticks;
      if (mydata->opinion_a > mydata->opinion_b && mydata->opinion_a > mydata->opinion_c && mydata->opinion_a > mydata->opinion_d) {
        set_opinion_a();
      }
      else if (mydata->opinion_b > mydata->opinion_a && mydata->opinion_b > mydata->opinion_c && mydata->opinion_b > mydata->opinion_d) {
        set_opinion_b();
      }
      else if (mydata->opinion_c > mydata->opinion_a && mydata->opinion_c > mydata->opinion_b && mydata->opinion_c > mydata->opinion_d) {
        set_opinion_c();
      }
      else if (mydata->opinion_d > mydata->opinion_a && mydata->opinion_d > mydata->opinion_b && mydata->opinion_d > mydata->opinion_c) {
        set_opinion_d();
      }
      else { // en cas d'égalité, on garde son opinion
        ...
        else if (mydata->flag_speaker == 3) {
          set_opinion_c();
        }
        else if (mydata->flag_speaker == 4) {
          set_opinion_d();
        }
        else printf("ERREUR MAJORITY_RULE \n");
      }
      vider_tableau_uid();
    }
  }
...
void loop() {
  ...
  if(kilo_uid == 0 && (kilo_ticks % intervalle_temps_kiloticks == 0 || kilo_ticks % intervalle_temps_kiloticks == 1) && kilo_ticks != 1) {
    ...
    if ((double)tableau_nbre_agents[2] / 2.0 <= nbre_theorique_d)
      qualite_consensus_d[dernier_indice_temps] = fabs((nbre_theorique_d - fabs(nbre_theorique_d - tableau_nbre_agents[3])) / nbre_theorique_d);
    else qualite_consensus_d[dernier_indice_temps] = 0.0;
      ...
      if(mydata->flag_nest) {     
        if (mydata->flag_listener == 1) {
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_a ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_a > mydata->opinion_b && mydata->opinion_a > mydata->opinion_c && mydata->opinion_a > mydata->opinion_d) {
              set_opinion_a();
              vider_tableau_uid();
            }
          }            
          ...
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_b ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_b > mydata->opinion_a && mydata->opinion_b > mydata->opinion_c && mydata->opinion_b > mydata->opinion_d) {
              set_opinion_b();
              vider_tableau_uid();
            }
          }  
        }
        else if (mydata->flag_listener == 3) {
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_c ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_c > mydata->opinion_a && mydata->opinion_c > mydata->opinion_b && mydata->opinion_c > mydata->opinion_d) {
              set_opinion_c();
              vider_tableau_uid();
            }
          }  
        }
        else if (mydata->flag_listener == 4) {
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_d ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_d > mydata->opinion_a && mydata->opinion_d > mydata->opinion_b && mydata->opinion_d > mydata->opinion_c) {
              set_opinion_d();
              vider_tableau_uid();
            }
          }  
...
void setup() // initialisation au tout début, une seule fois
{
  ...
  /* seuil du MR */
  mydata->seuil_majority_rule = seuil_majority_rule;
...
char *cb_botinfo(void) {
  ...
  if (mydata->state == DISSEMINATION) {
      ...
      p += sprintf (p, "Opinion b : %d ", mydata->opinion_b);
      p += sprintf (p, "Opinion c : %d ", mydata->opinion_c);
    	p += sprintf (p, "Opinion d : %d \n", mydata->opinion_d);
...
int16_t cb_lighting(double x, double y) {
  if (mydata->state == DISSEMINATION) {
    ...
    if (mydata->flag_speaker == 1) { // aller vers le site a
      double light_x = 500.0;
      double light_y = 400.0;
      double dist_x = pow(light_x + x, 2);
      double dist_y = pow(light_y + y, 2);
      double dist_c = sqrt(dist_x + dist_y);
      return (int16_t)dist_c;
...
json_t *json_state() {
  ...
  json_t* seuil = json_integer(mydata->seuil_majority_rule);
