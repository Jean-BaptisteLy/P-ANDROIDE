...
static const uint8_t seuil_majority_rule = 40;
...
void affichage_resultats() {
  if (consensus_atteint == tableau_nbre_agents[0] || consensus_atteint == tableau_nbre_agents[1] || consensus_atteint == tableau_nbre_agents[2]) {
...
void majority_tule() {
  ...
  if (kilo_ticks > (mydata->last_time_update + nb_ticks - 93)) { // during the last three seconds
    if (kilo_ticks > (mydata->last_time_update + nb_ticks)) {
      mydata->last_time_update = kilo_ticks;
      if (mydata->opinion_a > mydata->opinion_b && mydata->opinion_a > mydata->opinion_c) {
        set_opinion_a();
      }
      else if (mydata->opinion_b > mydata->opinion_a && mydata->opinion_b > mydata->opinion_c) {
        set_opinion_b();
      }
      else if (mydata->opinion_c > mydata->opinion_a && mydata->opinion_c > mydata->opinion_b) {
        set_opinion_c();
      }
...
void loop() {
  ...
  if(kilo_uid == 0 && (kilo_ticks % intervalle_temps_kiloticks == 0 || kilo_ticks % intervalle_temps_kiloticks == 1) && kilo_ticks != 1) {
    ...
    qualite_consensus_global[mydata->indice_temps] = (qualite_consensus_a[mydata->indice_temps] + qualite_consensus_b[mydata->indice_temps] + qualite_consensus_c[mydata->indice_temps]) / NBRE_OPTIONS;
      ...
      if(mydata->flag_nest) {     
        if (mydata->flag_listener == 1) {
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_a ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_a > mydata->opinion_b && mydata->opinion_a > mydata->opinion_c) {
              set_opinion_a();
              vider_tableau_uid();
            }
          }            
          ...
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_b ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_b > mydata->opinion_a && mydata->opinion_b > mydata->opinion_c) {
              set_opinion_b();
              vider_tableau_uid();
            }
          }  
          ...
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_c ++;
            mydata->cpt_voisins ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_c > mydata->opinion_a && mydata->opinion_c > mydata->opinion_b) {
              set_opinion_c();
              vider_tableau_uid();
            }
          }  
