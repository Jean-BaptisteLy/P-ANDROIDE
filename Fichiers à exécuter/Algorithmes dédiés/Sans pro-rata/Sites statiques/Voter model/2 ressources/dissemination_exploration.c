...
static const uint8_t seuil_voter_model = 40;
...
void voter_model() {
  ...
  if (kilo_ticks > (mydata->last_time_update + nb_ticks - 93) || mydata->cpt_voisins >= mydata->seuil_voter_model) { // during the last three seconds
    if (kilo_ticks > (mydata->last_time_update + nb_ticks) || mydata->cpt_voisins >= mydata->seuil_voter_model) {
