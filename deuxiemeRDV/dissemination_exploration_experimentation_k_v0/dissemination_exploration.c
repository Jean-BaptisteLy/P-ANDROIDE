...
void explore() {
  ...
  if (kilo_ticks > (mydata->last_time_update + mydata->exploration_time)) {
    ...
    if(mydata->flag_a_speaker) set_color(RGB(3,0,0));
    else set_color(RGB(0,0,3));
  }
...
void k_unanimity() { /* GESTION DES OPINIONS DE SES VOISINS */
  uint32_t nb_ticks;
  ...
  if (kilo_ticks > (mydata->last_time_update + nb_ticks - 93)) { // during the last three seconds
    if (kilo_ticks > (mydata->last_time_update + nb_ticks)) {
      ...
      if(mydata->flag_a_speaker) set_opinion_a();
      else if (!mydata->flag_a_speaker) set_opinion_b();
      else printf("ERREUR IMPOSSIBLE");
      vider_tableau_uid();
    }
  }
...
void loop() {
  if(mydata->state == DISSEMINATION) {
    if (mydata->new_message) {
      ...
      nest();
      if(mydata->flag_nest) {      
        if(mydata->flag_a_speaker && !mydata->flag_a_listener) { // si je suis de l'opinion a et je recois un message d'un agent d'opinion b
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_b ++;
            if(mydata->opinion_b >= mydata->k) {
              set_opinion_b();
              vider_tableau_uid();
            }
          }            
        }
        else if(!mydata->flag_a_speaker && mydata->flag_a_listener) { // si je suis de l'opinion b et je recois un message d'un agent d'opinion a
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_a ++;
            if(mydata->opinion_a >= mydata->k) {
              set_opinion_a();
              vider_tableau_uid();
            }
          }  
...
int16_t cb_lighting(double x, double y) {
  if (mydata->state == DISSEMINATION) {
    ...
    if(mydata->flag_a_speaker == 0) { // aller vers le site b
