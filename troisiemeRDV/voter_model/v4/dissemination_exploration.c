...
static const uint32_t intervalle_temps_graphe = 18600; // en kiloticks
/*
...
void loop() {
  ...
  if(kilo_uid == 0) {
    if(kilo_ticks % intervalle_temps_graphe == 0) {
      double rapport;
      uint8_t valeur_theorique;
      double val_absolue;
      rapport = (double)quality_of_site_a  / ((double)quality_of_site_a + (double)quality_of_site_b);
      valeur_theorique = rapport * (double)consensus_atteint;
      val_absolue = valeur_theorique - fabs(valeur_theorique - (double)consensus_site_a);
      abscisses[indice_abscisses] = val_absolue / valeur_theorique;
      indice_abscisses ++;
      printf("valeur : %f\n",abscisses[indice_abscisses]);
    }

    if (kilo_ticks == 360000) {
      printf("######################### TEMPS LIMITE ATTEINT ######################### \n");
      printf("uid : %d \n",kilo_uid);
      uint8_t i;
      for (i = 0; i < 37; i++) {
        if (i == 0) printf("[%f, ", abscisses[i]);
        else if (i == 36) printf("%f]", abscisses[i]);
        else printf("%f, ", abscisses[i]);
      }
      exit(0);
    }
  }
