...
void transition_tetu() {
  if (mydata->changements_opinions == mydata->seuil_changements_opinions) {
    ...
    if (mydata->changements_opinions_a > mydata->changements_opinions_b) {
      ...
      mydata->quality_of_site = 10; // ON MODIFIE ICI
      set_color(RGB(1,1,3));
...
void nest() {
    ...
    if (mydata->intensity <= 510) { // si je ne suis pas dans le nid (représenté par l'intensité de la lumière venant du nid)
      mydata->flag_nest = 1; // je ne prends pas les avis de mes voisins en compte
    }
    else mydata->flag_nest = 0; // je prends les avis de mes voisins en compte
      ...
      if(mydata->flag_nest && mydata->tetu == 0) {     
        if (mydata->flag_listener == 1) {
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_a ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_a > mydata->opinion_b) {
              set_opinion_a();
              vider_tableau_uid();
            }
          }            
          ...
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_b ++;
            if(mydata->cpt_voisins >= mydata->seuil_majority_rule && mydata->opinion_b > mydata->opinion_a) {
              set_opinion_b();
              vider_tableau_uid();
            }
          }  
...
void setup() // initialisation au tout début, une seule fois
{
  ...
  /* seuil du MR */
  mydata->seuil_majority_rule = 15;

  /* AGENT TÊTU */

  /* seuil du nombre de changements d'avis avant de devenir un agent têtu */
  /* ATTENTION : il faut que le nombre soit IMPAIR */
  mydata->seuil_changements_opinions = 3;
  mydata->changements_opinions = 0;
  mydata->changements_opinions_a = 0;
  mydata->changements_opinions_b = 0;
  mydata->tetu = 0;

  if(kilo_uid < 25) mydata->seuil_changements_opinions = 3;
  else if (kilo_uid < 50) mydata->seuil_changements_opinions = 4;
  else if (kilo_uid < 75) mydata->seuil_changements_opinions = 5;
  else if (kilo_uid < 100) mydata->seuil_changements_opinions = 6;
  else printf("ERREUR IMPOSSIBLE \n");
