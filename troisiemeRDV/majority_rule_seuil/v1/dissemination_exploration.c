...
static const uint8_t max_turning_ticks = 160; /* constant to allow a maximum rotation of 180 degrees with \omega=\pi/5 */
static const uint8_t max_straight_ticks = 80; /* set the \tau_m period to 2.5 s: n_m = \tau_m/\delta_t = 2.5/(1/32) */
...
void set_motion( motion_t new_motion_type ) {
   if( mydata->direction != new_motion_type ){
      ...
      mydata->direction = new_motion_type;
   }
}

/*
void random_walk() { // A AMELIORER
  uint8_t temp;
  temp = rand() % 4;
  set_motion(temp); 
}
...
void random_walk(){
   ...
   case RIGHT:
      ...
      break;
   case FORWARD:
      if( kilo_ticks > mydata->last_motion_ticks + max_straight_ticks ) {
         ...
         mydata->turning_ticks = rand_soft()%max_turning_ticks + 1;
      }
      break;
   case STOP:
   default:
...
void set_behavior() {
  ...
  if (kilo_ticks >= mydata->stepTicks + STEP_TIME) {
    ...
    else if (mydata->behavior_state == ANTI_PHOTOTAXIS) {
      if (mydata->intensity > mydata->lastIntensity) {
        mydata->lastIntensity = mydata->intensity;
        mydata->lastTicks = kilo_ticks;
        mydata->search_state = BETTER;
    ...
    else if (mydata->behavior_state == RANDOM_WALK) {
      random_walk();
    }
    else printf("ERREUR IMPOSSIBLE SET_BEHAVIOR");
  ...
  if (kilo_ticks > (mydata->last_time_update + nb_ticks - 93)) { // during the last three seconds
    if (kilo_ticks > (mydata->last_time_update + nb_ticks)) {
      ...
      if (mydata->opinion_a > mydata->opinion_b) {
        ...
        else printf("ERREUR IMPOSSIBLE MAJORITY_RULE \n");
      }
      vider_tableau_uid();
    }
  }
      ...
      if(mydata->flag_nest) {     
        if (mydata->flag_listener == 1) {
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_a ++;
            if(mydata->opinion_a >= mydata->seuil) {
              set_opinion_a();
              vider_tableau_uid();
              //mydata->state = EXPLORATION;
            }
          ...
          if(!mydata->flag_voisin_deja_rencontre) {
            mydata->opinion_b ++;
            if(mydata->opinion_b >= mydata->seuil) {
              set_opinion_b();
              vider_tableau_uid();
              //mydata->state = EXPLORATION;
            }
...
char *cb_botinfo(void)
{
  ...
  if (mydata->state == DISSEMINATION) {
    ...
    if (mydata->behavior_state == RANDOM_WALK) p += sprintf (p, "RANDOM_WALK");
    else if (mydata->behavior_state == PHOTOTAXIS) p += sprintf (p, "PHOTOTAXIS");
    else if (mydata->behavior_state == ANTI_PHOTOTAXIS) p += sprintf (p, "ANTI_PHOTOTAXIS");
    else p += sprintf (p, "Intensity: %d \n", mydata->intensity);
  }
  else if (mydata->state == EXPLORATION) {
  ...
  else printf("ERREUR IMPOSSIBLE BOTINFO \n");
...
int16_t cb_lighting(double x, double y) {
  ...
  else printf("ERREUR IMPOSSIBLE CB_LIGHTING\n");
  return 0;
}
...
int16_t boundaries(double x, double y, double * dx, double * dy) {
  double d = x;
  double d2 = y;
  if (mydata->state == DISSEMINATION) { //&& mydata->flag_nest == 1) {
  ...
  printf("ERREUR IMPOSSIBLE BOUNDARIES\n");
  return 0;
}
